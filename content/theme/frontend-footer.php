	<footer class="container-fluid text-center footer pt-2 pb-2">
		<a href="#myHome" id="topnotchaer" title="To Top">
			<span class="fas fa-chevron-circle-up"></span>
		</a>
		<p class="m-0">Copyright &copy; <?php echo date('Y'); ?> <a href="//<?php echo $cwebzite ?>" target="_blank" title="Visit <?php echo $cmpnyname ?>"><?php echo $cmpnyname ?></a>. All Rights Reserved.</p>
		<p>Build by: <span><?php echo $build_by ?></span></p>
	</footer>
	<script src="<?php echo $domainhome; ?>assets/js/script.js"></script>
	<script src="<?php echo $domainhome; ?>content/theme/<?php echo $themename; ?>/assets/js/script.js"></script>
	<script src="<?php echo $domainhome; ?>content/theme/<?php echo $themename; ?>/assets/js/custom-script.js"></script>
</body>
</html>
<!-- Lludvick Novechskie PHP CMS Framework -->